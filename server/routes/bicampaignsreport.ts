...
const bicampaignsreportRouter: Router = Router();

bicampaignsreportRouter.post("/", (request: Request, response: Response) => {
...
function getData(inputData, cb){
  ...
  dbFactory.get()
      ...
      .then(function (conn) {
          ...
          req.query(`select * from  [dbo].[CitrinePowerBISettings]  where (MerchantBranchId=${merchantBranchId} and ReportType='campaignreport') or ReportType='noreport' ORDER BY ReportType desc`)
...
export { bicampaignsreportRouter };
